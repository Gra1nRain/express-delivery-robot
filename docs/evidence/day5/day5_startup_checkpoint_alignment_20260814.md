...
用户发布初始位姿后，粗锚点成功应用为 revision 2，锚点节点报告车辆静止、路线未使能；校准状态长期停在 `COLLECTING / unusable alignment observation`。这证明阻塞不在 `/initialpose` 或静止判定。

现场单帧 `/scan` 只有约 36–64 个有效点。连续静止样本的搜索修正在约 `43–45 cm` 平移和 `0–5.75 deg` yaw 之间跳动，部分触碰默认 `45 cm` 搜索边界，最佳中位残差约 `6.2–10.7 cm`。6 帧融合后仍给出边界处约 `45 cm / 1 deg`，因此“只增加融合帧数”假设被否定。

扩大仅用于诊断的搜索窗口后，粗锚点本身的基线中位残差约 `5.4–5.6 cm`、10 cm 内点率约 `61.7–65%`；搜索却沿重复地图几何给出 `86–90 cm` 平移，只把中位残差改善约 `1.4 cm`、内点率改善约 `1.7%`。这是重复墙面几何下的歧义最优解，不能应用到 TF。

修复策略：若连续静止扫描表明当前粗锚点已经满足“基线中位残差不超过 `8 cm` 且 10 cm 内点率不少于 `50%`”，校准状态机把有效修正视为单位变换，保留人工粗锚点；仍执行 5 帧确认、revision 更新和 3 帧复核。只有粗锚点不满足该条件时，才继续采用原有的稳定候选修正、搜索触边拒绝和硬限幅策略。

新增现场指标回放回归，覆盖歧义大修正被拒绝、粗锚点保持以及最终进入 READY。电脑端 `python -m pytest -q tests` 为 `301 passed`。该修复尚待小车端重新构建、重启终端5并重新发布初始位姿后验证。
