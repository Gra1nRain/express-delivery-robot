...
## 控制验证整线实车事故复盘（2026-07-22）

- `b2fd838` 控制验证整线实车在 `debug_control_validation_trajectory.yaml` 的 index `84-88` 附近发生货架碰撞，用户遥控接管后测试终止。
- bag 显示自主阶段进入该区域前 `/control/status=TRACKING`、`/safety/event=SAFE_ACTIVE`，横向误差约 `0.012-0.018m`，`/cmd_vel.linear.x` 约 `0.19m/s`；这次事故不是主要由控制器跑偏造成。
- `t≈124.52s` 后底盘 `control_mode` 从 CAN 变为 mode `3`，safety 因 `remote_not_ready` 输出零速度；之后 `/odom` 中的倒车运动属于遥控接管后的运动。
- `/avoidance/proximity_status` 在自主阶段一直是 `clear` 且 `point_count=0`。结合 FAST-LIO `preprocess.blind=0.5m` 和旧 `stop_distance_m=0.55m`，原近场停车门只有很窄的可用检测带，且旧扇区没有覆盖完整车体前方横向安全包络。
- 已修复：近障门控改为“前向扇区 OR 车体前方矩形走廊”，`stop_distance_m=0.85`、`lateral_half_width_m=0.45`，并增加配置级回归测试。证据与结论见 `docs/evidence/day5/day5_control_validation_collision_analysis_20260722.md`。
