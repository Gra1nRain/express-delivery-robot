...
- 路线跟踪发生非瞬时状态异常时先进入 `FAULT_HOLD` 并保持零速；状态、局部路径和
  停车请求连续恢复正常 `0.5 s` 后，自动继续当前检查点，不重置任务或路线进度。
...
| 输出 | `/mission/arm_vision_preload` | `std_msgs/Bool` | 红绿灯识别结束后幂等请求常驻机械臂节点加载 YOLO |
...
- 抓取和放置之外统一使用行驶/待机关节位姿
  ...
  待机回位不作为小车挥旗发车的前置判断。回位命令复用最新夹爪开度，因此启动时
  保持当前开度、抓取后保持夹紧、放置后保持张开。成功任务只有在
  回位完成后才向主状态机返回成功；失败任务会尽力回位，但不覆盖原始失败原因。
...
- 预热点后发生 `FAULT_HOLD` 的现场卡死已形成回归测试；稳定状态自动恢复逻辑已通过
  PC 自动测试，仍需下一轮实车复核从预热点继续到真实停车点。
...
- `1.0 m` 红绿灯预触发距离和 `120/90 s` 机械臂总超时是初始配置，需在无底盘运动的
  单模块计时后复核。
- 启动自动待机已于 2026-08-18 完成无底盘真机械臂验证，到位最大关节误差为
  `0.0010 rad`；装卸后的统一行驶位姿返回和持物回位仍只有 PC 单元测试证据。
