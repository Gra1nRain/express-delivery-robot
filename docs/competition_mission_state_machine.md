...
- 抓取和放置之外统一使用行驶/待机关节位姿
  `[0.005760, 0.289742, -0.565347, -0.081856, 0.045605, 0.092502] rad`。初始位姿由
  操作员在启动前设置，不增加启动前置判断；每个装卸任务结束时由适配器自动回位。
  回位命令复用最新夹爪开度，因此抓取后保持夹紧、放置后保持张开。成功任务只有在
...
- `1.0 m` 红绿灯预触发距离和 `120/90 s` 机械臂总超时是初始配置，需在无底盘运动的
  单模块计时后复核。
- 统一行驶位姿自动返回目前只有 PC 单元测试证据，尚未同步到车端，也未进行真机械臂
  回位或持物回位验证。
