...
当前每个抓取点最多尝试 `2` 次。任意一次成功后不再重试；第一次失败时机械臂先恢复统一行驶位姿，再在同一抓取点开始第二次，第二次仍失败才由状态机决定是否前往后点。每次成功尝试内部仍保持“实物定位、预抓、抓取、夹紧验证、恢复统一行驶位姿”的顺序。
...
- 红绿灯提前 `1.0 m` 只预热识别、到真实停止点才停车的拆分逻辑已通过 PC
  自动测试，仍需下一轮实车复核；每个装卸点的抓取和放置都最多尝试 `2` 次，抓取总超时 `180 s`，放置总超时 `90 s`。
