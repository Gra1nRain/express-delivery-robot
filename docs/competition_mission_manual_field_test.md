...
前后放置点各只执行一次 Action。到点后机械臂调整到放置观察位，仅在相机中心识别一次，不执行左右摆臂；成功后完成放置并恢复统一行驶位姿，失败后由状态机前往后点或继续后续路线。
...
- 红绿灯提前 `1.0 m` 只预热识别、到真实停止点才停车的拆分逻辑已通过 PC
  自动测试，仍需下一轮实车复核；每个装卸点的抓取和放置 Action 都只尝试 `1` 次，抓取总超时 `180 s`，放置总超时 `90 s`。
