...
- 当前比赛轨迹 artifact 的首点是 `x=-0.376 m`、`y=0.112 m`、`yaw=0.02 rad`；该值不是固定物理起点的 `/initialpose`。
...
先把小车准确停回桌面单独导航测试使用的已标记固定物理起点和方向线。这里发布的是该物理起点经 RViz 人工对齐后的 `map` 位姿，不是轨迹 YAML 的首点；两者不得混用。终端 B：
...
该值仅适用于上述同一已标记物理起点。发布后至少等待 `1 s`，并在 RViz 中确认雷达扫描与静态地图重合；若不重合，不要发车，也不要用轨迹首点覆盖。若场地或起点发生变化，必须重新人工对齐并记录新的物理起点位姿。
...
抓取纸片只在相机中心识别，不执行左右摆臂；未识别到时保持当前观察/预抓位姿持续识别，直到识别成功或抓取总超时。纸片识别成功后保持面向瓶子和物块的位姿，不返回统一行驶位姿。前后抓取点各只执行一轮“中心、左侧、右侧”实物扫描，每个观察方向只识别一次；找到目标后立即预抓、抓取和夹紧验证，成功后才恢复红绿灯观察/统一行驶位姿。该轮失败后也不在同一点重试，由状态机前往后点或放弃装货。

前后放置点各只执行一次 Action。到点后机械臂保持夹爪闭合并调整到放置观察位，按“中心、左侧 `+10°`、右侧 `-10°`”扫描放置牌；任一视角识别成功后立即停止扫描并执行放置。前点整轮未找到时，夹爪保持闭合并恢复统一行驶位姿，小车必须前往后点再执行同样一轮扫描，因此放置阶段总共有前后两个停车点的识别机会，不会从前点失败直接去终点。找到放置点后，识别、回中心、移动至释放高度期间夹爪始终闭合，只在实际释放步骤才打开。

机械臂标注画面默认以 `15 Hz`、传感器低延迟 QoS 和深度 `1` 发布。实物搜索尚未产生新标注时显示最新实时相机画面，不再反复显示上一阶段的纸片截图；YOLO 标注在每次推理完成后立即替换。该改动提升监看刷新率和降低旧帧排队，但 YOLO 的实际推理频率仍受模型与车载算力限制，不等同于固定 `15 Hz` 推理。
...
- 红绿灯提前 `1.0 m` 只预热识别、到真实停止点才停车的拆分逻辑已通过 PC
  自动测试，仍需下一轮实车复核；每个装卸点的抓取和放置 Action 都只尝试 `1` 次，放置前后点各执行一轮中心/左右扫描，抓取总超时 `180 s`，放置总超时 `90 s`。
