...
| OPEN | 在线局部避障 / 代价图 | 基于实时点云/障碍物和静态地图的局部轨迹重规划、绕障或至少可证明安全停车 | 已有离线实现：body 点云 -> 局部 costmap/scan -> 静态图融合 -> 带总体轨迹偏离代价的短视距 Hybrid A* -> Day4 参数化 -> MPPI 动态切换；hard-stop 仅作失效兜底。合成桌体绕行测试通过，尚未车端/实车验证 | 作为当前 P0 主问题；在整线正式验收、任务闭环或再次靠近货架前必须完成无运动拓扑、真实货架低速绕行和失效停车验证 | 离线合成绕行、短段跑通、整线贴合轨迹或 proximity hard-stop 均不等于实车避障能力完成 |
