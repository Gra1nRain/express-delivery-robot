...
| OPEN | 在线局部避障 / 代价图 | 基于实时点云/障碍物和静态地图的局部轨迹重规划、绕障或至少可证明安全停车 | 当前只有冻结全局轨迹 + MPPI 跟踪 + 近场 hard-stop；不会像旧 `ranger_delivery_mission` 那样用局部代价图/clearance gate 主动避开货架 | 作为当前 P0 主问题；在整线正式验收、任务闭环或再次靠近货架前必须有独立验证 | 短段跑通、整线贴合轨迹、或 proximity hard-stop 不等于避障能力完成；不得因为后续点云时间戳、启动流程或调参问题转移/关闭本项 |
