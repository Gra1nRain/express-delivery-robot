...
固定物理起点的 2026-08-15 RViz 人工对齐基线为
`(-0.466, 0.464, 0.000)`。本轮对照测试按车头起始方向前移
`0.050 m`；由于起始航向为 `0 rad`，只增加 `map x`：
...
人工对齐基线来自 `fastlio_anchor` 对最后一次 `/initialpose` 的接收记录；当前 `-0.416 m` 是明确的测试偏移量，不是重新测量结果。连续轨迹文件的第一个参考点仍为 `(-0.376, 0.112, 0.020)`；固定初始位姿表示实车实际放置位置，不等同于修改轨迹几何。每次测试必须先把小车停回已标记的同一物理位置和朝向，再发布下面记录的固定初始位姿。
