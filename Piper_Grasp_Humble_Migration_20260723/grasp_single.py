...
BOTTLE_FORWARD_EXTRA_M = float(
    os.getenv("WRIST_BOTTLE_FORWARD_EXTRA_M", "0.055")
)
...
class PiperController(Node):
    ...
    def resolve_post_close_hold_gripper(self, commanded_closed_m):
        if not math.isclose(
            float(commanded_closed_m),
            CLOSE_GRIPPER_M,
            rel_tol=0.0,
            abs_tol=1e-12,
        ):
            self.get_logger().warn(
                "忽略非零夹爪闭合目标，强制使用 0.0000m。"
            )
        self.get_logger().info(
            "夹爪闭合后保持零宽度目标；"
            "实际接触开度只用于状态显示，不再反写控制目标。"
        )
        return float(CLOSE_GRIPPER_M)
