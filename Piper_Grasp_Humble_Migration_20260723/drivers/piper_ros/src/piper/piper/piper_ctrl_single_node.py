...
class PiperRosNode(Node):
    ...
    def prepare_move_j_callback(self, request):
        ...
        if not self.GetEnableFlag():
            self.get_logger().warning(
                "Ignored prepare_move_j because the arm is not enabled"
            )
            return

        self.get_logger().info(
            "Preparing CAN MOVE_J control (no position target)"
        )
    ...
    def pos_callback(self, pos_data):
        ...
        if(self.GetEnableFlag()):
            ...
            gripper = int(
                clip(
                    round(pos_data.gripper * 1000 * 1000),
                    0,
                    MAX_GRIPPER_POSITION_UM,
                )
            )
    ...
    def joint_callback(self, joint_data):
        ...
        if len(joint_data.position) >= 7:
            ...
            joint_6 = int(
                clip(
                    round(joint_data.position[6] * 1000 * 1000)
