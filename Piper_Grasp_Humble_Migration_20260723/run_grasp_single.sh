...
# 方块只有在检测框完整离开画面边缘并连续两帧稳定时，才允许使用较低阈值。
# 瓶子仍沿用上面的 0.50 单帧定位策略，避免改变已验证的瓶子效果。
export WRIST_BLOCK_COMPLETE_DETECTION_CONFIDENCE="${WRIST_BLOCK_COMPLETE_DETECTION_CONFIDENCE:-0.30}"
